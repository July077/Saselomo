$(function() {
	$('#coreShow').on('click', '.roleAddBtn, .roleModifyBtn', showEditModal);
	$('#coreShow').on('click','.roleDeleteBtn', showDeleteModal);
	$('#coreShow').on('click','.roleSearchBtn', doGetObjects);
	doGetObjects();//发起ajax请求,将服务器传入的数据显示在页面
});
//显示删除模态框
function showDeleteModal(){
	$('#deleteModal .deletePromptMessage').html('确定删除此角色信息么?');//填写内容
...
//	console.log('showEditModal');
	var url = 'role/newModifyModalUI.do';
	var title;
	if($(this).hasClass("roleAddBtn")){
		title="新建角色信息";
	}
	if($(this).hasClass("roleModifyBtn")){
		title="修改角色信息";
		//将要修改的记录的id值绑定到模态框上
		//目的时通过一个模块实现添加或更新操作 
...
//			console.log(JSON.stringify(result));
			//设置表格tabody中的内容
			setTableRows(result.data.roles);
...
//	console.log(clients);
				   ...
				   '<td>'							+
				    	'<div class="bs-example tooltip-demo">' +
				    		'<div class="bs-example-tooltips">'	+
					          	'<button data-toggle="tooltip" data-placement="top" title="详情" class="btn roleDetailsBtn [rowDetailsBtn]">'	+
					          		'<span class="glyphicon glyphicon-eye-open text-primary"></span>'					+
					          	'</button>'																				+
					          	'<button data-toggle="tooltip" data-placement="top" title="修改" class="btn roleModifyBtn [rowModifyBtn]">'	+
					          		'<span class="glyphicon glyphicon-pencil" style="color: #FFD793;"></span>'			+
					          	'</button>'																				+
					          	'<button data-toggle="tooltip" data-placement="top" title="删除" class="btn roleDeleteBtn [rowDeleteBtn]">'	+
	...
	for(var i in roles){
		...
		tr.data('id', roles[i].id);
		//将td模板追加到tr
		tr.append(template
				.replace('[no]', token)
				.replace('[role]', roles[i].role)
				.replace('[id]', roles[i].id)
				.replace('[description]', roles[i].description)
				.replace('[parentId]', roles[i].parentId)
				.replace('[createdTime]', roles[i].createdTime)
